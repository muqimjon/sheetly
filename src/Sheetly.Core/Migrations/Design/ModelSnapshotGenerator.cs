...
public class ModelSnapshotGenerator
{
		...
		string contextName)
	{
		...
		sb.AppendLine($"{Indent}{Indent}{Indent}LastUpdated = {CSharpHelper.FormatLiteral(snapshot.LastUpdated)}");
		sb.AppendLine($"{Indent}{Indent}}};");
		sb.AppendLine();
	...
	{
		sb.AppendLine($"{indent}snapshot.Entities[\"{EscapeString(entity.TableName)}\"] = new EntitySchema");
		sb.AppendLine($"{indent}{{");
		sb.AppendLine($"{indent}{Indent}TableName = \"{EscapeString(entity.TableName)}\",");
		sb.AppendLine($"{indent}{Indent}ClassName = \"{EscapeString(entity.ClassName)}\",");
		sb.AppendLine($"{indent}{Indent}Namespace = \"{EscapeString(entity.Namespace)}\",");
		...
		if (entity.Relationships.Count > 0)
		{
			...
			for (int i = 0; i < entity.Relationships.Count; i++)
			{
				...
				sb.AppendLine($"{indent}{Indent}{Indent}new RelationshipSchema {{ FromProperty = \"{EscapeString(rel.FromProperty)}\", ToTable = \"{EscapeString(rel.ToTable)}\", Type = RelationshipType.{rel.Type} }}{comma}");
			}
	...
	private void GenerateColumn(StringBuilder sb, ColumnSchema column, string indent, bool isLast)
	{
		...
		sb.AppendLine($"{indent}{{");
		sb.AppendLine($"{indent}{Indent}Name = \"{EscapeString(column.Name)}\",");
		sb.AppendLine($"{indent}{Indent}PropertyName = \"{EscapeString(column.PropertyName)}\",");
		sb.AppendLine($"{indent}{Indent}DataType = \"{EscapeString(column.DataType)}\",");
		...
		if (!string.IsNullOrEmpty(column.ForeignKeyTable))
			sb.AppendLine($"{indent}{Indent}ForeignKeyTable = \"{EscapeString(column.ForeignKeyTable)}\",");

		if (!string.IsNullOrEmpty(column.ForeignKeyColumn))
			sb.AppendLine($"{indent}{Indent}ForeignKeyColumn = \"{EscapeString(column.ForeignKeyColumn)}\",");
		...
		if (column.MinValue.HasValue)
			sb.AppendLine($"{indent}{Indent}MinValue = {column.MinValue.Value.ToString(CultureInfo.InvariantCulture)}m,");

		if (column.MaxValue.HasValue)
			sb.AppendLine($"{indent}{Indent}MaxValue = {column.MaxValue.Value.ToString(CultureInfo.InvariantCulture)}m,");
